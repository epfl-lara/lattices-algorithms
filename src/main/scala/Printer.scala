...
object Printer {
  ...
  def pretty(f: Formula): String = s"${f.uniqueKey}"+ (f match
    case Variable(id) => s"x$id"
  ...
  def pretty(f: OcbslAlgorithm.NormalFormula): String = s"${f.uniqueKey}"+ (f match
  ...
  def pretty(f: OcbslAlgorithm.NoOrFormula): String = s"${f.uniqueKey}"+ (f match
